            ...
            for line in coordinateReader:
                ...
                for i in range(len(matrix["durations"])):
                    for j in range(len(matrix["durations"][i])):
                        ...
                        #Helicopter time consist of dispatch delay, helicopter dispatch delay and flight to rendezvous site
                        #Both got dispatch delay and it doesn't need to be taken account of
                        if(matrix["durations"][i][j]+ onSceneTime + groundUnitToLVOsite > groundUnitToLVOsite + helicopterDelay + 10 + allCopterData[i]["FlyingFromBaseToRendez"]):
                            ...
                            #Now let's see if the patient reaches Tays with overall time.
                            #Overall time consist of dispatch delay, route to LVOsite, 10 minutes for EM personnel to alert helicopter, flying from base to rendezvous, 
                            #transfer delay, flight to Tays, transferring patient to ED
                            #If overall time is under 120 minutes, patient reaches Tays within time limit

                            if(dispatchDelay + groundUnitToLVOsite + 10 + helicopterDelay + allCopterData[i]["FlyingFromBaseToRendez"] + transferDelay + allCopterData[i]["FlyingFromRendezToED"] + rooftopDelay < 120):
                                if( dispatchDelay + groundUnitToLVOsite + 10 + helicopterDelay + allCopterData[i]["FlyingFromBaseToRendez"] + transferDelay + allCopterData[i]["FlyingFromRendezToED"] + rooftopDelay < fastestTime ):
                                    fastestTime = dispatchDelay + groundUnitToLVOsite + 10 + helicopterDelay + allCopterData[i]["FlyingFromBaseToRendez"] + transferDelay + allCopterData[i]["FlyingFromRendezToED"] + rooftopDelay
                                    helicopterLimits = True

                #Adding +1 to if helicopter is limiting factor                
                if( helicopterLimits == True ):
                    helicopterLimitsCounter += 1
                #else adding +1 to ground unit limiting factor
                else:
                    groundLimits += 1
                #Write fastest time to end of the row
                line.append(fastestTime)
                #Write if the helicopter is limiting agent
                line.append(helicopterLimits)

                #Write the data in results file
                writer.writerow(line)

                #Print out how many percents are done.
                print(lineNumber/13285*100)
